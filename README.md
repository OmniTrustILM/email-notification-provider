...
A template written before escaping arrived may still carry `?html`, and keeps working: the built-in is dropped as the template is parsed, which leaves the value to be escaped on its way out, so the message is what it always was. The one use that cannot be dropped is where a further built-in reads the escaped value, as in `${variable?html?upper_case}`. That is refused when the notification is rendered, and the error names the edit: write `?esc?markup_string` in place of `?html` and close the expression with `?no_esc`. A content template that will not render is refused when the notification instance is saved, and any already stored in that state is named in the log at startup, with the same reason, so neither has to be found through a notification that failed to arrive. The Subject is plain text and its values are inserted as they are.
